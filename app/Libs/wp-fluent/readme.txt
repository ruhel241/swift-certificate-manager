...
global $wpdb;
new \SwiftCertificateManagerFluent\Connection($wpdb, ['prefix' => $wpdb->prefix], 'DB');
```
...
```PHP
new \SwiftCertificateManagerFluent\Connection($wpdb, ['prefix' => $wpdb->prefix], 'MyAlias');
```
...
$db = new \SwiftCertificateManagerFluent\QueryBuilder\QueryBuilderHandler($connection);
