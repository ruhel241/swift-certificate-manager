jQuery(document).ready(function($) {
    ...
    var swiftCertificateManager = {
        requestCertificateHandle(forms) {
            ...
            jQuery.post(ajaxurl, {
                action: 'swiftcm_public_ajax', // WordPress expects 'action' to be passed for AJAX hooks
        ...
        verifyCertificateHandle() {
            ...
            .then(function(response) {
                if (response.success) {
                    ...
                    $('.swiftcm-verify-certificate-message')
                        .show()
        ...
        fireCustomEvent(paymentMethod, response, forms) {
            window.dispatchEvent(new CustomEvent(`swiftcm_payment_next_action_${paymentMethod}`, {
                detail: { form: forms, response },
            }));
        },

        requestPaymentCheckbox() {
            const $paymentCheckbox = $('#swiftcm_payment_checkbox');
            const $paymentMethod   = $('.swiftcm_payment_method');
