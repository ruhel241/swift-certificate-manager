...
export default class SwiftCertificateManager {
        ...
        Vue.mixin({
            ...
            methods: {
                ...
                getDisplayShortCode(display, slug) {
                    if (display === 'box') {
                        return `[swifcema box-slug="${slug}"]`;
                    }
                    if (display === 'choice') {
                        return `[swifcema choice-slug="${slug}"]`;
                    }
                },
                getCopyUrlLink(link) {
                    return link;
    ...
    request(method, route, data = {}) {
        const url = `${window.swifcemaAdminVars.rest.url}/${route}`;

        const headers = { 'X-WP-Nonce': window.swifcemaAdminVars.rest.nonce };
